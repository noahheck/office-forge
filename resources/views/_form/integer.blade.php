...
<div class="form-group {{ ($required ?? false) ? 'required' : '' }}">
    ...
    @if ($details ?? false)
        - {{ $details }}
    @endif
    <input type="text" class="integer-field form-control {{ ($error ?? false) ? 'is-invalid' : '' }}" name="{{ $name }}" id="{{ $name }}" placeholder="{{ $placeholder ?? '' }}" value="{{ $value ?? '' }}" {{ ($autofocus ?? false) ? 'autofocus' : '' }} {{ ($required ?? false) ? 'required' : '' }}>
</div>
